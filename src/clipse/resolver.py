...
def _collect_selected_keys(section: Mapping[str, Any]) -> List[str]:
    # Interpret keys present in options/positionals mapping
...
def _apply_refs(doc: Dict[str, Any]) -> Dict[str, Any]:
    # Walk objects/actions and expand $ref shallow-merging overrides
...
def resolve_config(raw: Dict[str, Any], *, env: Optional[Mapping[str, str]] = None) -> ResolutionResult:
    """
    Resolve a clipse config for explanation/generation:
      - validate against core schema
      - expand local $ref and merges
      - render {{vars.*}} and {{id}} placeholders
      - collect constraint issues for each action/object and report
    """
